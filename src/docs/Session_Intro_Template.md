...
_Last updated: July 2026 — deploy-command revision. Key changes from v3.1:_
- _Section 7 instruction 10 added: after delivering deployable code changes, give the git add/commit/push command on one line, unprompted_

_Key changes from v3.0 (April 2026, cap-efficiency revision):_
...
10. After delivering any file(s) that change deployable app code (not
    docs/contract-only changes), immediately follow with the git deploy
    command on one line, without being asked, e.g.:
      git add -A && git commit -m "<session id>: <short summary>" && git push
    Skip this for contract-only or documentation-only sessions — nothing
    deployable changed.
```
