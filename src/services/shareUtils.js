...
import { parRelative } from '../pages/scorecard/scorecardUtils.js';
...
function buildShareHtml(r, ar, bank, breakdown, matchPayouts, logoDataUri, orientation = 'landscape') {
  ...
  const indicatorHtml = (level) => {
    ...
    // eagle or double_bogey — siblings relative to same 26px wrapper, matching SVG coords.
    // outer path at 2.5px, inner path at 4.5px = 2px between paths, hairline visible gap (same as ScoreGrid SVG).
    ...
    return `<div style="${outer}"></div><div style="${inner}"></div>`;
  };
  ...
  const scoreCell = (pi, h) => {
    ...
    return `<td style="text-align:center;font-size:10px;padding:0;"><div style="position:relative;display:inline-flex;align-items:center;justify-content:center;width:26px;height:26px;line-height:1;"><span style="display:block;line-height:1;padding-bottom:1px;">${g}</span>${ind}${dots}</div></td>`;
  };
